...
pub fn parse_idx(data: &[u8]) -> IdxData {
...
pub fn parse_palette(palette: &str) -> [Rgb<u8>; 16] {
    let segments = palette.split(",");
    let mut palette = [Rgb::<u8>([0, 0, 0]); 16];
    for (i, segment) in segments.enumerate() {
        hex::decode_to_slice(segment.trim(), &mut palette[i].0).unwrap();
    }
    return palette;
}

pub fn parse_frame(idx: &IdxData, file_data: &[u8]) -> RgbaImage {
    let _file_size = u16::from_be_bytes([file_data[0], file_data[1]]);
    let control_offset = u16::from_be_bytes([file_data[2], file_data[3]]);

    let control = parse_control(&file_data, control_offset as usize);
    return parse_data(&idx.palette, control, &file_data);
}
...
fn parse_control(data: &[u8], mut cursor: usize) -> ControlData {
            ...
            match command {
                ...
                0x05 => {
                    ...
                    let coordinates = Coordinates {
                        x1: u16::from_be_bytes([data[cursor + 1], data[cursor + 2]]) & 0xFFF >> 4,
                        x2: u16::from_be_bytes([data[cursor + 2], data[cursor + 3]]) & 0xFFF,
                        y1: u16::from_be_bytes([data[cursor + 4], data[cursor + 5]]) & 0xFFF >> 4,
...
fn read_rle(nibble_stream: &mut NibbleStream) -> Rle {
...
fn parse_data(
    ...
    let color_palette = control.color_palette.expect("Missing color palette");
    let alpha_palette = control.alpha_palette.expect("Missing alpha palette");
    let coordinates = control.coordinates.expect("No coordinates for subtitles");
    ...
    let offsets = control.rle_offsets.unwrap();
...
impl<'a> NibbleStream<'a> {
    ...
    pub fn take_nibble(&mut self) -> u8 {
        let start = self.data[self.cursor / 2];
        if self.cursor % 2 == 0 {
            self.cursor += 1;
            return start >> 4 & 0xF;
        } else {
    ...
    /// Keep in mind, the cursor sits on u4 boundaries, so to use
    /// it on a u8 slice, it must be divided by 2.
